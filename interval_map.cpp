...
typename std::map<K,V>::iterator it_begin =m_map.lower_bound(keyBegin);
typename std::map<K,V>::iterator it_end = m_map.lower_bound(keyEnd);
...
if(it_begin==m_map.begin()){
    ...
    before_it_begin = std::prev(temp)->second;
}
...
if(it_end==m_map.end()){
    ...
    after_it_end = std::prev(temp)->second;
}
else{
    // K supports only < operator
