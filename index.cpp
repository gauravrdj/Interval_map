...
typename std::map<K,V>::const_iterator it_begin =m_map.lower_bound(keyBegin);
typename std::map<K,V>::const_iterator it_end = m_map.lower_bound(keyEnd);

typename V before_it_begin;
if(it_begin==m_map.begin()){
    before_it_begin = val_begin;
}
else{
     typename std::map<K,V>::iterator temp = it_begin;
    before_it_begin = (*--temp).second;
}

typename V after_it_end;
if(it_end==m.begin()){
...
if(it_end==m.end()){
    typename std::map<K,V>::iterator temp = m_map.end();
    ...
    if((*it_end).first==end){
     after_it_end = (*it_end).second;
        ...
        typename std::map<K,V>::iterator temp = it_end;
        after_it_end = (*--temp).second;
    }
}
}
...
if(after_it_end==val){
    if(it_end!=m_map.end() && (*it_end).first==end){
        m_map.erase(it_end);
    }
}
else{
 ...
 int main(){
   ...
   interval_map<int,char>Map('A');
    ...
    for (const auto& entry : referenceMap) {
        ...
        char actualValue = Map.getValue(key); // You may need a getValue() function in your inter_map class
